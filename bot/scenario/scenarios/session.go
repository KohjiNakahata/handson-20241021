...
func (s *SessionScenario) handleOnGetSessionInfoBySessionType(payload []byte) {
	...
	logger.Debugu(s.GetUserID(), "Get Session Info By Session Type: %d, ID: %s, Members: %d/%d, Members: %v, Owner: %s", sessionType, sessionID, currentMembers, maxMembers, memberIDs, ownerID)
}
