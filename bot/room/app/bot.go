...
func Run() {
	parseArgs()
	logger.Info("bot args", "host", host, "bots", bots, "authInterval", authInterval, "roomSize", roomSize, "packetInterval", packetInterval, "packetSize", packetSize, "logLevel", logLevel, "protocol", protocol)
	spawnBots()
	for {
...
func printMetrics() {
	for _, bot := range bm.bots {
		logger.Debug("bot status", "bot.uid", bot.uid, "bot.state", bot.state, "bot.room", bot.room, "bot.broadcastSendCnt", bot.broadcastSendCnt.Load(), "bot.broadcastRcvCnt", bot.broadcastRcvCnt.Load())
	}
	logger.Info("-----------------------------------------------------")
	logger.Info("bot metrics", "joinedBotCnt", bm.getJoinedCnt(), "rcvCntTotal", bm.getBroadcastReceiveCntTotal(), "rcvAvg", bm.getBroadcastReceiveAvg(), "sendCntTotal", bm.getBroadcastSendCntTotal(), "sendAvg", bm.getBroadcastSendAvg(), "joinedRoom", bm.getJoinedRooms())
	logger.Info("-----------------------------------------------------")
	bm.resetCnt()
}
func spawnBots() {
...
func newBot(id string) *bot {
	...
	if err != nil {
		logger.Error("Auth error", "bot.uid", id, "err", err)
		return nil
	}
	logger.Debug("eResponse", "eResp.Sid", eResp.Sid, "eResp.EncryptionKey", eResp.EncryptionKey, "eResp.EncryptionIV", eResp.EncryptionIV, "eResp.EncryptionMacKey", eResp.EncryptionMacKey)
...
func searchAndJoin(bot *bot) {
	...
	roomCli.OnJoin(func(success bool, createdTime uint) {
		joinedCnt.Add(1)
		logger.Debug("OnJoin", "bot.uid", bot.uid, "success", success, "createdTime", createdTime)
		if success {
			bot.state = STATUS_BROADCAST
		}
	})
	roomCli.OnCreate(func(success bool, name string, createdTime uint) {
		logger.Info("OnCreate", "bot.uid", bot.uid, "success", success, "createdTime", createdTime)
		if success {
			bot.state = STATUS_BROADCAST
		}
	})
	roomCli.OnMemberLeave(func(message []byte) {
		logger.Debug("OnMemberLeave", "bot.uid", bot.uid, "message", message)
	})
